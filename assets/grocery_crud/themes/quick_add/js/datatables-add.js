var quickAddStatus = false,
    fieldName = segment[3],
    tableName = null,
    tableFieldReff = null,
    tableLabeldReff = null;
...
(function ($) {
    ...
    $(document).ready(function () {
        if (parent.$.fancybox.getInstance()) {
            ...
            $('<button type="button" class="fancyClose btn-php btn btn-danger btn-flat pull-right"><i class="fa fa-close"></i> Kapat</button>').insertAfter('#form-button-save');
...
function updateSelectBox(primaryKey) {
    return new Promise(function (resolve, reject) {
        var $parentBody = parent.$('body'),
            $inputBox = $parentBody.find('#' + fieldName + '_input_box'),
            $inputElem = $inputBox.find('select'),
            $relatedTable = $('#relatedTable'),
            tableName = $relatedTable.data('name'),
            tableFieldReff = $relatedTable.data('field'),
            tableLabeldReff = $relatedTable.data('label');

        $.get(site + '/quick/getRow/' + tableName + '/' + primaryKey, function (result) {
            var data = $.parseJSON(result);
            $inputElem.append($('<option value="' + data[tableFieldReff] + '" selected="selected">' + data[tableLabeldReff] + '</option>')).trigger("chosen:updated");
        }).done(function () {
            resolve({
                text: 'Yeni kayıt listeye eklendi.',
                type: 'success'
            });
        }).fail(function () {
            reject({
                text: 'Bilinmeyen bir hata meydana geldi. Lütfen sistem yetkilisi ile irtibata geçiniz.',
                type: 'error'
            });
        }).always(function () {
                    ...
                    $('#crudForm').ajaxSubmit({
                        ...
                        success: function (result) {
                            ...
                            if (data.success) {
                                ...
                                if (quickAddStatus) {
                                    updateSelectBox(data.insert_primary_key).then(function (res) {
                                        $('.field_error').removeClass('field_error');
                                        new PNotify({
                                            text: data.success_message + res.text,
                                            type: res.type
                                        });
                                        clearForm();
